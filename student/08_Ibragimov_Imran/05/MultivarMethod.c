...
struct DoubleLinkedList* TwoWayChainInit(int num, ...) {
	if (num == 0) return NULL;
	else {
		struct DoubleLinkedList* myList = (struct DoubleLinkedList *) malloc(sizeof(struct DoubleLinkedList));
		if (myList == NULL) exit(1);
...
void main() {
	...
	struct DoubleLinkedList* myList2 = TwoWayChainInit(5, 6, 62, 725, 3, 1);
	LinkedListDirectionPrint(myList2->first, RIGHT);
	myList2 = TwoWayChainInit(1, 6);
	LinkedListDirectionPrint(myList2->first, RIGHT);
	myList2 = TwoWayChainInit(5, 5, 15151515, 5, 5, 5);
	LinkedListDirectionPrint(myList2->first, RIGHT);
}